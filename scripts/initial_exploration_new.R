...
modelsummary(list(lm_vrm_9, lm_vrm_0.1, lm_vrm_0.05), # lm_vrm_2, lm_vrm_3, lm_vrm_4, lm_vrm_5, lm_vrm_6, lm_vrm_7, lm_vrm_8, lm_vrm_9), 
...
route_reference <- df_all_log %>%
  filter(year == 2025, month == 11) %>% #changed
  group_by(route_id) %>%
  summarise(
...
# ggsave(filename = "data/Nashville/output/allroutes_ridership.png", plot = allroutes_ridership, width = 11, height = 7.5, units = "in")
